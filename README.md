...
Currently, the [STAYGE](http://stayge.io/) wallet is supporting only the [ICON](https://m.icon.foundation/?lang=en) as a underlying blockchain network. To interface with the ICON network, it implemented the NodeJs version of the ICON SDK([python](https://github.com/icon-project/icon-sdk-python), [java](https://github.com/icon-project/icon-sdk-java)) based on the [ICON JSON-RPC specification](https://github.com/icon-project/icon-rpc-server/blob/master/docs/icon-json-rpc-v3.md). In the future, the [STAYGE](http://stayge.io/) Network will be provided for the [STAYGE](http://stayge.io/) services.
